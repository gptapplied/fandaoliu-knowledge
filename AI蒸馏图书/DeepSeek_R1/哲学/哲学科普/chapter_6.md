...
当你在深夜浏览购物网站时，算法已经根据历史记录将酸奶和燕麦片加入推荐列表；当你在社交媒体点赞宠物视频时，内容推送系统正在为你的「猫奴」人格标签增添权重。这些日常场景背后，是数以亿计的数据点构成的决策预测模型。通过贝叶斯定理 $P(A\lvert B) = \frac{P(B\lvert A)P(A)}{P(B)}$ 构建的概率网络，科技公司不仅能推测用户行为，甚至能在个体意识到选择倾向前，就预判其潜在决策路径。
